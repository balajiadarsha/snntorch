...
class Leaky(LIF):
    ...
    * :math:`β` - Membrane potential decay rate
    ...
    def _build_state_function(self, input_, mem):
        ...
        elif self.reset_mechanism_val == 3:  # no reset, pure integration
            state_fn = self._base_state_function(input_, mem)
        elif self.reset_mechanism_val == 2:  # abalaji: refractory reset
            state_fn = self._base_state_function(
                input_, mem - self.reset * self.threshold * self._refractory_factor
            )
        return state_fn
